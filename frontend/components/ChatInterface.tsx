...
export default function ChatInterface({
  ...
  const streamResponse = async (
    ...
    try {
      ...
      errorMsg[errorMsg.length - 1].content = `Error: ${
  ...
  return (
    <main className="flex h-screen bg-[#131314] text-white flex-col p-4">
      <div className="flex-1 overflow-y-auto max-w-3xl mx-auto w-full space-y-6 pt-10">
          ...
          >
            <div
              className={`max-w-[80%] p-4 rounded-2xl ${
                m.role === "user"
                  ? "bg-[#2e2f30]"
                  : "bg-transparent border border-[#2e2f30]"
              }`}
            >
        ...
        {isTyping && (
          <div className="text-gray-400 animate-pulse text-sm">Schreibt...</div>
        )}
      </div>
      ...
      <div className="max-w-3xl mx-auto w-full pb-8">
        <form
          ...
          className="relative bg-[#1e1f20] rounded-3xl px-6 py-4 flex items-center border border-[#333] focus-within:border-[#4285f4] transition-all"
        >
          <input
            ...
            disabled={isTyping}
          />
          <button type="submit" className="text-xl" disabled={isTyping}>
            🚀
          </button>
        </form>
      </div>
