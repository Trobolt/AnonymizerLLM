...
export default function Sidebar({
  ...
  return (
    <aside
      ...
      } bg-[#1e1f20] h-screen flex flex-col transition-all duration-300`}
    >
      ...
      <div className="flex-1 flex flex-col min-h-0">
        {" "}
        {/* min-h-0 ist wichtig für Flex-Scrolling */}
        {/* Scrollbare Liste */}
          ...
          {!isCollapsed && (
            <div className="space-y-1">
              {(chatIds || []).map((chatId) => (
                ...
                >
                  <span className="truncate text-sm pr-2">Chat {chatId}</span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      onDeleteChat(chatId);
